...
async def lookup_summary(acc):
    summoner_name = acc
    summoner_name_encoded = urllib.parse.quote(summoner_name)
    page = f"https://euw.op.gg/summoner/userName={summoner_name_encoded}"
...
async def opgg(summoner_name: str = Path(None, title="Summoner name of player"), fetch_match_history: bool = False):
    if summoner_name == "''":
        tasks = [
            asyncio.create_task(lookup_summary("V9 Altbert")),
            asyncio.create_task(lookup_summary("Alberner Albert")),
            asyncio.create_task(lookup_summary("Albert Number 1"))
        ]
        ...
        wr = data['winrate'].split(" ")[2]
        lp = data['lp'].replace(" ", "")
        if fetch_match_history:
            history = parse_html_streak(html)
            return f"{summoner_name}; {data['rank']} {lp}; mmr:{data['mmr']}; {wr}:{data['games']}; {history}"
        return f"{summoner_name}; {data['rank']} {lp}; mmr:{data['mmr']}; {wr}:{data['games']}"

# backend für frontend
@app.get("/api/{summoner_name}")
async def fetch_summoner_info(summoner_name: str, response: Response):
  summoner_name_encoded = urllib.parse.quote(summoner_name)
  page = f"https://euw.op.gg/summoner/userName={summoner_name_encoded}"
  html = await get_page_cached(page)
  data = parse_html(html)
